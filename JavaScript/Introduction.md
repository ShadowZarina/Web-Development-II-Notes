...
while (i < 5) { // 2. Check the condition
  ...
  i++; // 3. Update the variable (crucial to avoid an infinite loop!)
}
```
- Do-while loop
...
```
do {
  // Code to execute at least once, and repeat if condition is true
} while (condition);
```

```
let i = 5; // 1. Initialize a variable

do {
  console.log("This will print once: " + i);
  i++; // 2. Update the variable
} while (i < 5); // 3. Check the condition (5 < 5 is false, so it stops here)
```
...
- The JavaScript Event Object is a built-in object automatically created by the browser whenever an event occurs in the Document Object Model (DOM).
- It acts as a messenger, delivering vital data about the action—such as which element was clicked, which key was pressed, or the mouse's exact coordinates.
- To access it, you pass it as a parameter (commonly named e or event) into your event listener's callback function. 
...
- Depending on the action, JavaScript passes a specialized event object containing unique properties tailored to that specific user interaction.

Examples of Event Objects:
1. Mouse
- (click, mousemove)
- MouseEvent
- clientX / clientY (viewport coordinates), button (which mouse button was clicked)
2. Keyboard (keydown, keyup)
- KeyboardEvent
- key (the character value), code (the physical key code), shiftKey (if Shift was held)
3. Form (submit, input)
- SubmitEvent / InputEvent
- inputType (how data was added), submitter (the button that triggered the form submit)
4. Touch (touchstart, touchend)
- TouchEvent
